...
def signup():
    ...
    if flag == True:
        ...
        cursor.execute("""insert into users (user_name, email,mobile, password_salt, password_hash, user_image) VALUES (%s, %s, %s,%s,%s,%s)""", (user_name, email,mobile, salt, password_hash,location))
        mysql.connection.commit()
        cursor.close()
...
def adminsignup():
    ...
    results = cursor.fetchall()
    for item in results:
        if str(admin_email) == str(item["admin_email"]):
            flag = True
            mysql.connection.commit()
            cursor.close()
    if flag == True:
        ...
        cursor.execute("""insert into admin (admin_name, admin_email,admin_mobile, admin_password_salt, admin_password_hash, admin_image) VALUES (%s, %s, %s,%s,%s,%s)""", (admin_name, admin_email,admin_mobile, salt, password_hash,location))
        mysql.connection.commit()
        cursor.close()
        return json.dumps("Registerd Successfully")
...
def adminlogin():
    ...
    cursor.close()
    for item in results:
        if str(admin_email) == str(item["admin_email"]) and str(item["admin_password_hash"]) == str(md5_hash(password+item["admin_password_salt"])):
            flag = True
            encoded_jwt=jwt.encode({"admin_id":item["admin_id"],"admin_name":item["admin_name"],"admin_email":item["admin_email"],"admin_mobile":item["admin_mobile"],"admin_image":item["admin_image"]}, 'secretkey', algorithm='HS256').decode("utf-8")
    if flag == True:
