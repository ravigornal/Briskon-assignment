...
export default class Home extends Component {
    ...
    pagination = (pageNo) => {
        let updatePage = this.state.next_page
    ...
    render() {
        ...
        return (
            ...
            <form className = 'float-right my-3 d-flex mr-5' onSubmit={this.hanleSubmit}>
                    ...
                    this.state.filteredData.map((e) => {
                            return(
                                <Link to={`/details/${e.prod_id}}`}>
                                    <div className="float-left ml-5 mt-5</div>">
                                            <div className="text-center"> 
                                                <img src = {`http://127.0.0.1:5000/${e.prod_image}`} style={{height:"250px", width:"400px"}} ></img>
                                                <h1>{e.prod_name}</h1>
                                                <h2>{e.prod_price}</h2>
                                            </div>
                                        
                                        <div>
                                            <button className="btn btn-primary offset-5 mt-3 mb-3">Buy Now</button>
                                            <button className="btn btn-dark ml-3">Cart</button>
                                        </div>
                                    </div>
                                 </Link>
                            )
                            ...
                            this.state.allData.map((e) => {
                                ...
                                <Link to={`/details/${e.prod_id}}`}>
                                    <div className="float-left ml-5 mt-5</div>">
                                        ...
                                        <div>
                                            <button className="btn btn-primary offset-4 mt-3 mb-3">Buy Now</button>
                                            <Link to="/admindash"><button className="btn btn-dark ml-3">Cart</button></Link>
                                        </div>
                                    </div>
                                </Link>
                                )
